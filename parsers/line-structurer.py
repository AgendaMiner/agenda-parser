...
def main():
	...
	# clean and extract info from the JSON agenda
...
def extractSectionNumber(line_text):
	...
	if re_starts_num.match(line_text) is not None:
		raw_num_string = re_starts_num.match(line_text).group()
	else:
		return False

	# clean the section number
	num_string = raw_num_string.strip() # remove whitespace
	num_string = num_string.replace(".", "") # remove dots
